...
def main() -> None:
    ...
    materials = {
        "frame_black": material("#151c20", "#080c0f", "#38434a", "dither", 4),
        "frame_dark": material("#222d33", "#0d1418", "#4d5c64", "dither", 3),
        "frame_blue": material("#2d424d", "#15242c", "#58717b", "dither", 4),
        ...
        "rubber": material("#17191b", "#060708", "#393d3e", "dither", 5),
        "cloth": material("#263a4d", "#111e2b", "#536f88", "dither", 4),
        "cloth_light": material("#49677f", "#23394c", "#7896ae", "dither", 3),
        "trouser_blue": material("#3c5a72", "#1c3144", "#7190a8", "dither", 3),
        "skin": material("#9e665d", "#53312f", "#d99a84", "dither", 3),
        "skin_light": material("#bd7b6d", "#6c3d38", "#e9aa90", "dither", 2),
        ...
        "shield": material("#713516", "#2d140b", "#b4662d", "dither", 4),
        "shield_light": material("#9b5223", "#4b2010", "#d8843e", "stripes", 3),
        "shield_dark": material("#49200f", "#1c0c08", "#7c3b1d", "dither", 5),
    ...
    add(box("pelvis_frame_core", "exo_pelvis", (0, 58, 0), (22, 9, 14),
            "load-bearing exoframe pelvis bridge", "frame_dark"), None, (0, 58, 0))
    add(box("pelvis_front_armor", "exo_pelvis", (0, 58, 7.4), (16, 6, 2.2),
            "front pelvis armor apron", "frame_blue", rotation=(8, 0, 0)),
        "pelvis_frame_core", (0, 58, 6.7))
    add(box("pelvis_rear_crossbar", "exo_pelvis", (0, 59, -7.0), (25, 4.5, 3.0),
            "rear hip load crossbar", "frame_black"),
        "pelvis_frame_core", (0, 59, -6.5))
    add(box("saddle_core", "pilot_pelvis", (0, 60.5, 1.5), (10, 5, 10),
            "operator saddle nested in frame", "rubber"),
        "pelvis_frame_core", (0, 60, 1.5))
    add(box("torso_frame_spine", "torso_frame", (0, 70, -4.8), (5, 19, 5),
            "central exoframe spine behind operator", "frame_black"),
        "pelvis_frame_core", (0, 62, -3.5))
    add(box("shoulder_frame_crossbar", "torso_frame", (0, 77, -2.5), (24, 5, 7),
            "broad equipment shoulder frame", "frame_dark"),
        "torso_frame_spine", (0, 76, -4))
    for side, sign in (("left", -1.0), ("right", 1.0)):
        add(box(f"{side}_pelvis_cheek", "exo_pelvis", (sign * 10.7, 58, 0), (4, 8, 12),
                f"{side} layered hip cheek", "frame_blue"),
            "pelvis_frame_core", (sign * 9.5, 58, 0))
        add(box(f"{side}_waist_brace", "torso_frame", (sign * 4.0, 66, -2), (8, 11, 6),
                f"{side} torso-frame waist brace", "frame_dark"),
            "torso_frame_spine", (sign * 2, 66, -3.5))
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        add(box(f"pilot_{side}_control_grip", f"pilot_{side}_arm", hand,
            ...
            fore, hand)
        add(box(f"pilot_{side}_control_stick", "torso_frame",
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        hip_block = add(box(f"{side}_exo_hip_block", f"{side}_exo_hip", hip, (9, 9, 11),
                            f"{side} powered hip housing", "frame_dark"),
        ...
        for face, z_value in (("front", 5.7), ("rear", -5.7)):
            add(box(f"{side}_hip_{face}_drum", f"{side}_exo_hip",
                    (hip[0], hip[1], z_value), (6.3, 6.3, 1.7),
        ...
        thigh_cube = segment_cube(f"{side}_exo_thigh_beam", f"{side}_exo_thigh",
                                  hip, knee, (4.8, 5.5), f"{side} primary powered thigh beam",
        ...
        add(box(f"{side}_thigh_outer_armor", f"{side}_exo_thigh",
                (thigh_mid[0] + sign * 3.3, thigh_mid[1], thigh_mid[2]), (3.2, 17, 8.0),
                f"{side} broad outer thigh armor plate", "frame_dark",
                rotation=thigh_rotation),
            thigh, (thigh_mid[0] + sign * 2.2, thigh_mid[1], thigh_mid[2]))
        add(box(f"{side}_thigh_warning_panel", f"{side}_exo_thigh",
                (thigh_mid[0] + sign * 5.0, thigh_mid[1], thigh_mid[2] + 0.5),
                (1.1, 8.5, 4.5), f"{side} thigh identification panel", "warning",
                rotation=thigh_rotation),
            f"{side}_thigh_outer_armor", (thigh_mid[0] + sign * 4.5, thigh_mid[1], thigh_mid[2] + 0.5))
        brace_mid = (thigh_mid[0] - sign * 2.8, thigh_mid[1], thigh_mid[2] - 2.8)
        add(box(f"{side}_thigh_inner_rail", f"{side}_exo_thigh", brace_mid, (2.4, 18, 2.6),
                f"{side} exposed inner thigh rail", "edge_steel", rotation=thigh_rotation),
            thigh, (thigh_mid[0] - sign * 2.3, thigh_mid[1], thigh_mid[2] - 2.0))

        knee_block = add(box(f"{side}_exo_knee_core", f"{side}_exo_knee", knee,
                             (7, 7, 7), f"{side} compact knee hinge inside open tread guard", "frame_black"),
                         thigh, knee)
        for face, z_value in (("front", knee[2] + 3.8), ("rear", knee[2] - 3.8)):
            add(box(f"{side}_knee_{face}_drum", f"{side}_exo_knee",
                    (knee[0], knee[1], z_value), (5.5, 5.5, 1.8),
        ...
        add(segment_cube(f"{side}_shin_rear_brace", f"{side}_exo_shin",
                         rear_brace_start, rear_brace_end, (1.9, 2.0),
        ...
        shin_cube = segment_cube(f"{side}_exo_shin_beam", f"{side}_exo_shin",
                                 knee, ankle, (4.5, 5.2), f"{side} lower powered shin beam",
        ...
        add(box(f"{side}_shin_outer_armor", f"{side}_exo_shin",
                (shin_mid[0] + sign * 3.1, shin_mid[1], shin_mid[2]), (3.0, 22, 7.3),
                f"{side} long outer shin armor", "frame_blue", rotation=shin_rotation),
            shin, (shin_mid[0] + sign * 2.2, shin_mid[1], shin_mid[2]))
        add(box(f"{side}_shin_inner_rail", f"{side}_exo_shin",
                (shin_mid[0] - sign * 2.8, shin_mid[1], shin_mid[2] - 2.5), (2.2, 22, 2.4),
        ...
        casing = add(segment_cube(f"{side}_shin_piston_casing", f"{side}_exo_shin",
                                  piston_start, piston_mid, (3.1, 3.1),
        ...
        for face, z_value in (("front", ankle[2] + 3.7), ("rear", ankle[2] - 3.7)):
            add(box(f"{side}_ankle_{face}_drum", f"{side}_exo_ankle",
                    (ankle[0], ankle[1], z_value), (4.8, 4.8, 1.5),
        ...
        ankle_lower = add(box(f"{side}_ankle_lower_segment", f"{side}_exo_ankle",
                              (ankle[0], 6.0, ankle[2] + 1.0), (7, 3, 8),
                              f"{side} lower segmented ankle collar", "frame_blue"),
                          ankle_block, (ankle[0], 7.5, ankle[2] + 0.5))
        restraint = add(box(f"pilot_{side}_boot_restraint", f"pilot_{side}_leg",
                            (sign * 16.5, 14.0, 2.5), (8.0, 2.4, 3.5),
                            f"lateral restraint coupling pilot {side} boot to powered ankle",
                            "brass"),
        ...
        for toe_index, offset_x in enumerate((-4.0, 0.0, 4.0), start=1):
            toe_name = add(box(f"{side}_toe_{toe_index}", f"{side}_exo_foot",
                               (sign * 22.0 + offset_x, 1.7, 16.5), (2.6, 3.4, 9),
    ...
    add(box("backpack_main", "backpack", (0, 78, -9), (16, 16, 7),
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        add(box(f"backpack_{side}_canister", "backpack", (sign * 9.5, 76, -12),
                (6.0, 20, 8), f"{side} distinct tall power cylinder", "frame_black"),
            "backpack_main", (sign * 7.5, 76, -10))
        add(box(f"backpack_{side}_cap_top", "backpack", (sign * 9.5, 86.2, -12),
                (7.0, 2.4, 9.0), f"{side} canister upper collar", "edge_steel"),
            f"backpack_{side}_canister", (sign * 9.5, 85.5, -12))
        add(box(f"backpack_{side}_cap_bottom", "backpack", (sign * 9.5, 65.8, -12),
                (7.0, 2.4, 9.0), f"{side} canister lower collar", "edge_steel"),
            f"backpack_{side}_canister", (sign * 9.5, 66.5, -12))
        add(box(f"backpack_{side}_mid_collar", "backpack", (sign * 9.5, 76, -12),
                (7.0, 2.0, 9.0), f"{side} power-cylinder center band", "brass"),
    ...
    shield_core = add(box("shield_core", "shield", shield_point(0, 0, 16), (3.0, 49, 2.5),
    ...
    for side, local_x in (("left", -15.7), ("right", 15.7)):
        ...
        edge_names[side] = add(
            box(f"shield_{side}_edge_rail", "shield", point, (2.2, 50, 2.0),
    ...
    for bay_index, local_x in enumerate((-10.8, -3.6, 3.6, 10.8), start=1):
        ...
        bay = add(box(f"shield_grille_bay_{bay_index}", "shield", point, (4.2, 50, 1.6),
        ...
        for rib_index, local_y in enumerate((-12, 0, 12), start=1):
            ...
            add(box(f"shield_bay_{bay_index}_rib_{rib_index}", "shield", rib_point,
                    (5.5, 1.0, 0.8), f"shield bay {bay_index} cellular cross rib {rib_index}",
    ...
    lower_points: tuple[Point, ...] = (
        (7, 69, 0), (12, 76, 7), shield_point(0, -10, 14.6)
    )
    ...
    spec = {
        ...
        "generation": {
            ...
            "algorithm": "multiview-pilot-inside-powered-frame-v1",
            "source_mesh": None,
            ...
            "pilot_exoframe_contacts": [
                {
                    "side": side,
                    "first": f"pilot_{side}_boot_restraint",
