...
constraint rank separately for all inputs and observed inputs. It also groups
near-duplicate observed frames by normalized masked-subject RGB content and
silhouette. Distinct filenames,
resolutions, timestamps, or claimed azimuths do not establish another camera
axis when the pixels are effectively the same. Hidden geometry is credited
only when the independent-content observed rank is at least two, and never
when proxy views alone establish depth.
...
For named production assets, the workflow treats web galleries and timestamped
gameplay video as topology research rather than optional inspiration. Record
exact sources in a cycle-local `research-sources.json`, and request another
perspective whenever the available material does not establish limb count,
wearer relationships, or mechanical attachments.
...
# Opt-in calibrated texture transfer after geometry and cameras are approved.
pip install -e '.[reference-projection]'
img2blockbench bake-reference-textures \
  ./red-panda.json \
  --views ./red-panda-texture-views.json \
  --style minecraft \
  --texture-density 1 \
  --palette-size 24 \
  --colors-per-material 4 \
  --atlas-size 1024 \
  --output ./red-panda-textured.json \
  --audit ./red-panda-texture-audit.json
...
# Archived full-checkout helper (not in the compact release). It also requires
# SciPy, unlike the production reference-projection command above.
pip install -e '.[reference-projection]'
...
`from-mesh` also writes `<id>.mesh-views.json` and a sibling
`<id>.mesh-views/` directory. The manifest hashes deterministic front, back,
left, right, top, bottom, and isometric review PNGs. Each image places the
source ray-albedo projection, fitted-cuboid projection, and silhouette-error
overlay side by side, so side or rear collapse is visible before approval. A
subsequent `build` verifies those hashes and copies the manifest and all seven
PNGs into both the build directory and deterministic ZIP; the delivered model
spec points at that localized copy. Portable copies retain the external mesh's
filename and SHA-256 as provenance but remove workstation paths and mark the
mesh itself as external rather than bundled.
...
`source_region` is rejected unless the face or its effective material supplies
a `source_texture`.

`bake-reference-textures` accepts a versioned manifest containing one or more
fixed perspective views. Each view must provide an exact-size foreground mask,
verified SHA-256 hashes, and the resolved camera intrinsics and extrinsics used
for visual review:
  ...
  "views": [
    {
      ...
      "mask": "front-mask.png",
      "mask_sha256": "64 lowercase hexadecimal characters",
      "camera": {
...
The baker tests every cuboid face texel against a foreground mask and a
frontmost-face z-buffer. If multiple views observe a texel, it chooses the most
front-facing view and breaks exact ties by view ID. Its default `minecraft`
style samples at one texel per model unit, removes photographic microdetail,
builds one bounded palette across the calibrated views, and limits each
semantic material to a small color ramp without dithering. Thus the source
controls markings and material layout without being pasted onto the model as a
photographic collage. Unobserved texels receive the authored material's solid
base color and are never reported as inferred source detail. Use `--style
source` only when literal source pixels are intentionally required. The
emitted per-face PNG patches are embedded in the model spec, and
`generation.texture_transfer` records coverage, style parameters, and
provenance. The command is opt-in, so existing procedural specs compile
byte-for-byte as before. Atlases up to 2048 pixels are supported.

New starter and untextured Three.js imports use a solid material by default.
Random dither is retained as a backwards-compatible pattern, but it is not a
substitute for observed color, panel boundaries, decals, or localized wear.

### Player-wearable exoskeletons

An authored model may declare a `wearable` contract when a visible reference
pilot is only a fit proxy for a replaceable Minecraft player. The contract
names the removable occupant bone subtree, retained shell root, player fit
transform, collision volume, and physical harness/control/stirrup attachment
points. A normal build then keeps the occupied source-comparison model and also
emits a scaled shell-only `.bbmodel`, `.geo.json`, texture, model spec, audit,
and Bedrock `.attachable.json` descriptor. This prevents a baked-in mannequin
from being mistaken for a wearable machine. The fit gate requires the authored
wearer height to resolve near the canonical 32-unit player height and its
anchor to resolve at the player origin. Locomotion animation and item
registration remain integration responsibilities for the target pack or mod.
...
The historical numbered routes converge on the same validated Minecraft model
specification and delivery bundle, but their upstream reasoning is separate.
...
named ray evidence from all six signed axes, source/model occupancy and depth
profiles for X/Y/Z, source coverage, model precision, voxel IoU,
one-voxel-tolerant surface F1, orthographic silhouette IoU, and opposing-boundary
depth error. A source-relative anti-pancake gate requires every source axis and
depth envelope to survive the cuboid fit while also bounding voxel overfill,
per-axis silhouette overfill, and depth-envelope inflation. It records
intrinsically thin source axes without rejecting legitimate plates or swords.
The fitted voxel union must retain a one-to-one mapping to the source's
6-connected components: merged components fail just like missing components.
Reconstruction also fails when the cuboid budget cannot represent every
retained component instead of silently deleting parts. Because it emits a
