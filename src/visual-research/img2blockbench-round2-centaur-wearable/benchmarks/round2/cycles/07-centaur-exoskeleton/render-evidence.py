...
from wearable_export import make_wearable_spec
...
def actual_inventory(spec: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "boot_restraints": sum(name.endswith("_boot_stirrup") for name in names),
...
def player_fit_preview(shell: dict[str, Any]) -> dict[str, Any]:
    """Insert a canonical 8x32 classic-player proxy into the exported shell."""
    preview = json.loads(json.dumps(shell))
    preview["materials"].update(
        {
            "player_skin": {
                "base": "#ad795b",
                "shade": "#754834",
    ...
    preview["bones"].append(
        {"name": "wearer_preview", "parent": "root", "pivot": [0, 0, 0]}
    )
    proxy_cubes = (
        ("wearer_head", (0, 28, 0), (8, 8, 8), "player head", "player_skin"),
        ("wearer_body", (0, 18, 0), (8, 12, 4), "player torso", "player_shirt"),
        ("wearer_left_arm", (-6, 18, 0), (4, 12, 4), "player left arm", "player_skin"),
        ("wearer_right_arm", (6, 18, 0), (4, 12, 4), "player right arm", "player_skin"),
        ("wearer_left_leg", (-2, 6, 0), (4, 12, 4), "player left leg", "player_trousers"),
        ("wearer_right_leg", (2, 6, 0), (4, 12, 4), "player right leg", "player_trousers"),
    )
    for name, center, size, role, material in proxy_cubes:
        preview["cubes"].append(
            {
                "name": name,
                "bone": "wearer_preview",
                "center": list(center),
    ...
    preview["quality_contract"]["target_cuboids"][1] = max(
        preview["quality_contract"]["target_cuboids"][1],
        len(preview["cubes"]),
    )
    return preview
...
def main() -> None:
    ...
    wearable = make_wearable_spec(spec)
    wearable_atlas, wearable_placements = build_texture(wearable)
    wearable_views = {
        "wearable-shell-isometric": render_view(
            wearable,
            (1.0, 0.8, 1.0),
            atlas_image=wearable_atlas,
            placements=wearable_placements,
            background=BACKGROUND,
        )
    }
    occupied = player_fit_preview(wearable)
    occupied_atlas, occupied_placements = build_texture(occupied)
    wearable_views.update(
        {
            "wearable-fit-front": render_view(
                occupied,
                (0.0, 0.0, 1.0),
                atlas_image=occupied_atlas,
                placements=occupied_placements,
                background=BACKGROUND,
            ),
            "wearable-fit-isometric": render_view(
                occupied,
                (1.0, 0.8, 1.0),
                atlas_image=occupied_atlas,
                placements=occupied_placements,
    ...
    for name, rendered in wearable_views.items():
        rendered.save(OUTPUT / f"{name}.png", optimize=True)
    ...
    for side in ("left", "right"):
        ...
        leg_nesting[side] = {
            ...
            "boot_restraint": f"{side}_boot_stirrup",
    ...
    for side in ("left", "right"):
        part_sizes = {
            "hip": cubes[f"{side}_exo_hip_block"]["size"],
            "thigh_armor": cubes[f"{side}_thigh_outer_armor"]["size"],
            "knee": cubes[f"{side}_exo_knee_core"]["size"],
            "shin_armor": cubes[f"{side}_shin_outer_armor"]["size"],
            "ankle": cubes[f"{side}_exo_ankle_core"]["size"],
            "foot": cubes[f"{side}_exo_ground_foot"]["size"],
        }
        powered_leg_mass[side] = {
            "part_sizes": part_sizes,
            "massive_powered_chain": (
                min(part_sizes["hip"]) >= 11.0
                and part_sizes["thigh_armor"][0] >= 5.0
                and part_sizes["thigh_armor"][2] >= 10.0
                and min(part_sizes["knee"]) >= 10.0
                and part_sizes["shin_armor"][0] >= 5.0
                and part_sizes["shin_armor"][2] >= 10.0
                and part_sizes["ankle"][0] >= 9.0
                and part_sizes["ankle"][2] >= 10.0
                and part_sizes["foot"][0] >= 16.0
                and part_sizes["foot"][2] >= 22.0
            ),
        }
    ...
    evaluation = {
        ...
        "texture_transfer": spec["generation"].get("texture_transfer"),
        "procedural_dither_materials": sorted(
            name
            for name, material in spec["materials"].items()
            if material["pattern"] == "dither"
        ),
        "wearable_export": {
            "target": spec["wearable"]["target"],
            "player_variant": spec["wearable"]["player_variant"],
            "occupant_mode": spec["wearable"]["occupant_mode"],
            "source_cuboids": len(spec["cubes"]),
            "shell_cuboids": len(wearable["cubes"]),
            "source_bones": len(spec["bones"]),
            "shell_bones": len(wearable["bones"]),
            "removed_occupant_bones": sorted(spec["wearable"]["occupant_bones"]),
            "attachment_points": spec["wearable"]["attachment_points"],
            "player_anchor_after_fit": [
                round(
                    coordinate * spec["wearable"]["fit"]["scale"]
                    + spec["wearable"]["fit"]["offset"][index],
                    6,
                )
                for index, coordinate in enumerate(
                    spec["wearable"]["fit"]["player_anchor"]
                )
            ],
            "shell_contains_pilot_bones": any(
                bone["name"] in set(spec["wearable"]["occupant_bones"])
                for bone in wearable["bones"]
            ),
        },
        ...
        "powered_leg_mass_audit": {
            "method": "minimum-part-cross-sections-v1",
            "sides": powered_leg_mass,
            "both_powered_chains_are_massive": all(
                record["massive_powered_chain"] for record in powered_leg_mass.values()
            ),
        },
        ...
        "views": [
            ...
            "wearable-shell-isometric.png", "wearable-fit-front.png",
            "wearable-fit-isometric.png",
        ],
