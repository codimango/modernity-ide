...
not reconstructed facts. The calibrated view contributes source-derived color
to 3,286 of 28,126 face texels (11.6831%) across 132 visible faces. This is a
Minecraft-style abstraction rather than a photographic skin: it uses one texel
per model unit, a deterministic 24-color shared palette, at most four colors
per semantic material, and no dithering. The other 24,840 texels retain their
solid semantic material colors. The source's redistribution rights are unknown,
so the private reference and copied build reference remain ignored.
...
PYTHONPATH=skill/img2blockbench/scripts python3 skill/img2blockbench/scripts/img2blockbench.py bake-reference-textures benchmarks/round2/cycles/06-falling-devil/model-spec.json --views benchmarks/round2/cycles/06-falling-devil/texture-views.json --output benchmarks/round2/cycles/06-falling-devil/model-spec.json --audit benchmarks/round2/cycles/06-falling-devil/texture-evidence.json --style minecraft --texture-density 1 --palette-size 24 --colors-per-material 4 --atlas-size 1024
...
fixed-frame silhouette overlay and comparison sheet. `texture-views.json`
freezes the source, mask, hashes, and camera; `texture-evidence.json` records
per-face observed/fallback coverage and the bounded palettes used by the bake.
