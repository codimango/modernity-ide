...
The final atlas uses the original olive/khaki photograph as the canonical
camouflage finish. A calibrated visible-face bake at one texel per model unit
contributes 1,261 source-observed texels across 161 faces (`5.1812%` of all face texels).
Those samples are prefiltered and reduced to a shared 24-color palette with at
most four colors per semantic material, without dithering. Every other texel
uses an authored olive, khaki, dark-green, or gunmetal panel base, so the result
reads as Minecraft camouflage rather than a photograph pasted across cuboids.
The white render remains geometry and six-leg topology evidence only; its
different paint scheme is not mixed into the texture atlas.

Two private references constrain the reconstruction: the canonical olive
...
PYTHONPATH=skill/img2blockbench/scripts python3 skill/img2blockbench/scripts/img2blockbench.py bake-reference-textures benchmarks/round2/cycles/02-militech-chimera/model-spec.json --views benchmarks/round2/cycles/02-militech-chimera/texture-views.json --style minecraft --texture-density 1 --palette-size 24 --colors-per-material 4 --atlas-size 1024 --output benchmarks/round2/cycles/02-militech-chimera/model-spec.json --audit benchmarks/round2/cycles/02-militech-chimera/texture-evidence.json
