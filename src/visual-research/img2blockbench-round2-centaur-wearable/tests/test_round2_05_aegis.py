...
class RoundTwoAegisX2Tests(unittest.TestCase):
    ...
    def test_three_view_texture_transfer_is_minecraft_styled_and_calibrated(self) -> None:
        """Gate bounded pixel art sourced from all three fixed cameras."""
        spec = json.loads((CYCLE / "model-spec.json").read_text(encoding="utf-8"))
        ...
        cameras = json.loads(
            (CYCLE / "render" / "perspective-cameras.json").read_text(
                encoding="utf-8"
            )
        )["cameras"]
        for view in manifest["views"]:
            self.assertEqual(cameras[view["id"]]["resolved_camera"], view["camera"])
        self.assertGreaterEqual(transfer["coverage"]["source_fraction"], 0.20)
        self.assertGreaterEqual(transfer["coverage"]["faces_with_source"], 320)
        self.assertEqual(870, transfer["coverage"]["total_faces"])
