...
SOURCE_OPTIONS = reference_texture.ReferenceTextureBakeOptions(style="source")
...
class ReferenceTextureTests(unittest.TestCase):
    ...
    def test_side_face_patch_accounts_for_blockbench_uv_flip(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            baked, _ = reference_texture.bake_reference_textures(
                self._spec(reference), manifest, SOURCE_OPTIONS
            )
    ...
    def test_minecraft_style_uses_low_density_bounded_material_palette(self) -> None:
        """Source layout survives as deliberate pixel art rather than photo collage."""
        with tempfile.TemporaryDirectory() as temp:
            folder = Path(temp)
            reference, manifest, _ = self._write_inputs(folder)
            spec = self._spec(reference)
            ...
            self.assertEqual(1, baked["texture"]["density"])
            self.assertEqual("minecraft", baked["texture"]["source_style"])
            self.assertEqual("solid", baked["materials"]["body"]["pattern"])
            self.assertLessEqual(len(set(south.get_flattened_data())), 2)
            self.assertTrue(audit["palette_quantization"])
            self.assertFalse(audit["style"]["dithering"])
            self.assertFalse(audit["style"]["source_pixels_embedded_verbatim"])
            self.assertLessEqual(
                len(audit["style"]["material_palettes"]["body"]), 2
            )
    ...
    def test_equal_incidence_uses_lexicographic_view_id(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            first, first_audit = reference_texture.bake_reference_textures(
                spec, manifest, SOURCE_OPTIONS
            )
            second, second_audit = reference_texture.bake_reference_textures(
                spec, manifest, SOURCE_OPTIONS
            )
            patch = img2blockbench.decode_source_texture(
                {
