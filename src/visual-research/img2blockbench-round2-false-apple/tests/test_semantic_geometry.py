...
class SemanticGeometryTests(unittest.TestCase):
    ...
    def test_ellipsoid_cutouts_subtract_before_deterministic_merging(self):
        cutout = semantic_geometry.EllipsoidCutout(
            center=(-4.5, 0.0, 0.0),
            radii=(3.5, 5.0, 20.0),
            exponent=2.0,
        )
        ...
        self.assertEqual(first, second)
        self.assertGreater(first.base_occupied_voxels, first.occupied_voxels)
        self.assertEqual(
            first.base_occupied_voxels - first.occupied_voxels,
            first.removed_voxels,
        )
        self.assertTrue(first.carved_faces)
        self.assertTrue(
            all(face in {"north", "east", "south", "west", "up", "down"}
                for _, face in first.carved_faces)
        )
        voxel_volume = math.prod(first.voxel_size)
        merged_volume = sum(math.prod(cube["size"]) for cube in first.cubes)
        self.assertAlmostEqual(
            first.occupied_voxels * voxel_volume,
            merged_volume,
            places=3,
        )
        ...
        with self.assertRaisesRegex(ValueError, "radii must be positive"):
            semantic_geometry.ellipsoid_cuboids(
                ...
                (8.0, 8.0, 7.0),
                (12, 12, 10),
                "bad carved test volume",
                "gold",
                cutouts=(
                    semantic_geometry.EllipsoidCutout(
                        center=(0.0, 0.0, 0.0),
                        radii=(0.0, 1.0, 1.0),
                    ),
                ),
