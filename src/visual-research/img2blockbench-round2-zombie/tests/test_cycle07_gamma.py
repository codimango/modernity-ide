...
class CycleSevenGammaTests(unittest.TestCase):
    ...
    def test_gamma_required_visual_evidence_and_clean_build_exist(self):
        ...
        for suffix in (
            ...
            self.assertTrue((build / f"gamma_abnormality.{suffix}").is_file())
        local_reference = build / "gamma_abnormality.reference.png"
        if local_reference.is_file():
            self.assertTrue(
                (build / "gamma_abnormality.zip").is_file(),
                "a local build with its private reference must include the deliverable ZIP",
            )
        # The reference copy and ZIP are intentionally ignored, so a clean clone
        # can only require the reproducible tracked artifacts checked above.
        audit = json.loads(
            (build / "gamma_abnormality.audit.json").read_text(encoding="utf-8")
        )
