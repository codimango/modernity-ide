...
class RoundTwoMilitechChimeraTests(unittest.TestCase):
    ...
    def test_chimera_uses_calibrated_direct_source_texture(self) -> None:
        """Gate two-view visible-face baking and solid unseen-face fallback."""
        spec = json.loads((CYCLE / "model-spec.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(
            {"alternate-white", "original-olive"},
        ...
        self.assertEqual(
            ...
            cameras["original-olive"],
        )
        self.assertEqual(
            json.loads(
                (CYCLE / "render" / "alternate-perspective-camera.json").read_text(
                    encoding="utf-8"
                )
            )["resolved_camera"],
            cameras["alternate-white"],
        )
        self.assertGreaterEqual(transfer["coverage"]["source_fraction"], 0.08)
        self.assertGreaterEqual(transfer["coverage"]["faces_with_source"], 350)
