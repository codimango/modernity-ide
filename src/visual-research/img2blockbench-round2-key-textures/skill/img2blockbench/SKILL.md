...
9. Run `img2blockbench build WORKSPACE/model-spec.json --output WORKSPACE/build`
   only after the shared cuboid scene passes the Three.js geometry review.
10. Open the emitted `.bbmodel` in Blockbench when available. Capture left,
   right, front, back, and isometric views plus bilateral head close-ups.
11. Correct the specification, rebuild, and re-render until the quality rubric
