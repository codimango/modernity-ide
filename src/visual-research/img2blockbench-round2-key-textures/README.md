...
front-facing view and breaks exact ties by view ID. Unobserved texels receive
the effective material's solid base color; they are never reported as inferred
source detail. The emitted per-face PNG patches are embedded in the model spec,
source quantization is disabled, and `generation.texture_transfer` records
coverage and provenance. The command is opt-in, so existing procedural specs
compile byte-for-byte as before. Atlases up to 2048 pixels are supported for
high-detail models.
