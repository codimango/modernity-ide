...
def _require_dependencies() -> tuple[Any, Any]:
    """Load overlap-audit dependencies with an actionable install hint."""
    global np, trimesh
    if np is None or trimesh is None:
        try:
            ...
            trimesh = None
            raise ValueError(
                "source-overlap audit requires numpy and trimesh; install "
                "img2blockbench[overlap-audit] or img2blockbench[all]"
            ) from exc
    return np, trimesh
...
def load_source_mesh(
    path: Path, canonical_transform: list[float]
...
def load_blockbench_mesh(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    for element in model.get("elements", []):
        ...
        triangles.append(np.asarray(CUBE_TRIANGLES, dtype=np.int64) + offset)
